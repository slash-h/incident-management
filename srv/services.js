...
class ProcessorService extends cds.ApplicationService {
  ...
  /** Custom Validation */
  async onUpdate(req) {
    let closed = await SELECT.one(1).from(req.subject).where`status_code = 'C'`
  ...
  async onCustomerRead(req) {
    ...
    //Expands are required as the runtime does not support path expressions for remote service
  ...
  async onCustomerCache(req, next) {
    ...
    if (newCustomerId && newCustomerId !== "") {
      ...
      const customer = await this.S4bupa.run(query);
